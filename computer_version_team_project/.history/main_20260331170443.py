...
while True:
    ...
    if ready_to_grab:
        ...
        # 每 60 帧（约 2 秒）提醒一次手的操作
        if frame_count % 60 == 0:
            movement_tips = [
                f"Keep holding the {target_object}!",
                "Lift the object higher!",
                "Good! Now rotate your hand slowly!",
                "Move the object away from you!",
                "Bring the object closer to you!"
            ]
            tip_index = (frame_count // 60) % len(movement_tips)
            speak_async(movement_tips[tip_index])
    else:
        guidance_parts = []
