...
class VoiceDetector:
    ...
    def __init__(self, yolo_class_names, default_target="cell phone", whisper_model="base"):
        ...
        self.max_retries = 5
        self.voice_retry_before_keyboard = 2
        self.min_match_score = 0.55
        self.auto_accept_score = 0.88
    ...
    def prompt_keyboard_choice(self, candidates, raw_text=None):
        """Use keyboard confirmation to avoid compounding speech recognition errors."""
        print("\nCandidate objects:")
        for index, (candidate, score) in enumerate(candidates, start=1):
            print(f"{index}. {candidate} ({score:.0%} match)")

        if raw_text:
            print(f"Recognized speech: {raw_text}")

        prompt = (
            "Choose 1-3, press Enter to accept 1, type r to retry voice, "
            "or type an object name manually: "
        )
        ...
        typed_target = self.extract_object_from_speech(user_input)
        return typed_target if typed_target in self.yolo_class_names else None

    def prompt_keyboard_fallback(self):
        """Fallback path when voice input is unreliable."""
        print("\nVoice recognition fallback")
        print(f"Type the object name directly, or press Enter for default: {self.default_target}")
        typed_text = input("Object name: ").strip().lower()

        if not typed_text:
            return self.default_target

        extracted = self.extract_object_from_speech(typed_text)
        if extracted in self.yolo_class_names:
            return extracted

        print("Input did not match a supported object. Using default target.")
        return self.default_target
    ...
    def get_target_object(self):
        ...
        for attempt in range(self.max_retries):
            if attempt >= self.voice_retry_before_keyboard:
                keyboard_target = self.prompt_keyboard_fallback()
                self.speak(f"OK, looking for {keyboard_target}")
                print(f"Confirmed target: {keyboard_target}\n")
            ...
            if score >= self.auto_accept_score:
                selected_target = self.prompt_keyboard_choice(ranked_candidates, raw_text=speech_text)
            else:
                print("Speech match is uncertain, please confirm with the keyboard.")
                selected_target = self.prompt_keyboard_choice(ranked_candidates, raw_text=speech_text)

            if selected_target:
                self.speak(f"OK, looking for {selected_target}")
                print(f"Confirmed target: {selected_target}\n")
                return selected_target

            print("Retrying voice selection.\n")
