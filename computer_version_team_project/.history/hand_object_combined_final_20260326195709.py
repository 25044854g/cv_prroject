...
while True:
    ...
    # 2. Pose detection (includes hand position)
    ...
    if results_pose[0].keypoints is not None:
        ...
        # Try right wrist (index 10)
        if len(keypoints) > 10:
            right_wrist = keypoints[10]
            if right_wrist[0] > 10 and right_wrist[1] > 10:
                hand_x = int(right_wrist[0])
                hand_y = int(right_wrist[1])
                if frame_count % 30 == 0:
                    print(f"Right wrist found at: ({hand_x}, {hand_y})")
        
        # If no right hand, try left wrist (index 9)
        if hand_x is None and len(keypoints) > 9:
            ...
            if left_wrist[0] > 10 and left_wrist[1] > 10:
                ...
                if frame_count % 30 == 0:
                    print(f"Left wrist found at: ({hand_x}, {hand_y})")
    ...
    if hand_x is not None and len(boxes) > 0:
        ...
        # Draw red circle at hand position
