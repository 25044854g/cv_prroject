...
# 先创建一个临时检测器获取 YOLO 类名列表
temp_detector = HandObjectDetector()
yolo_class_names = list(temp_detector.yolo_model.names.values())
...
# 用语音确认的目标初始化检测器
hand_detector = HandObjectDetector(target_object=target_object)
