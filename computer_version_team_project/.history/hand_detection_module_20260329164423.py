...
class HandObjectDetector:
    ...
    def process_frame(self, frame):
        ...
        if hand_x is not None and target_box is not None:
            ...
            hand_depth_info = self.estimate_hand_depth(detection_result.hand_landmarks[0], width, height)
            object_depth_info = self.estimate_object_depth(box, width, height)
            
            hand_depth_info = {
                'depth': hand_depth_info[0],
                'size': hand_depth_info[1]
            }
            object_depth_info = {
                'depth': object_depth_info[0],
                'size': object_depth_info[1]
            }
            
            depth_result = self.is_hand_in_front_of_object(hand_depth_info, object_depth_info)
            ...
            result_info['is_in_front'] = depth_result['is_in_front']
        else:
            if hand_x is None:
