...
for attempt in range(MAX_RETRIES):
    ...
    # Step 3: 语音询问确认
    confirm_text = listen_once(f"Do you want to find {extracted}? Please say yes or no.")
    ...
    if "yes" in confirm_text or "yeah" in confirm_text or "yep" in confirm_text or "correct" in confirm_text or "right" in confirm_text:
        ...
        tts_engine.say(f"OK, looking for {target_object}")
        tts_engine.runAndWait()
        print(f"Confirmed target: {target_object}\n")
        break
    else:
        print("Let's try again.\n")
        tts_engine.say("OK, let's try again.")
        tts_engine.runAndWait()
